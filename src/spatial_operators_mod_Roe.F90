MODULE spatial_operators_mod
  ...
  implicit none
    ...
    subroutine spatial_operator(stat,tend)
      ...
      do iPatch = ifs, ife
        !$OMP PARALLEL DO PRIVATE(i,eigenvalue_x,b,c,mtx_x,ip1,im1,jc,dE,dFlux)
        do j = jts, jte
          do i = its-1, ite
            eigenvalue_x(1) = stat%uc(i,j,iPatch) - sqrt( mesh%matrixIG(1,1,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ) )
            eigenvalue_x(2) = stat%uc(i,j,iPatch) + sqrt( mesh%matrixIG(1,1,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ) )
            
            b = sign(1.,eigenvalue_x(1))
            c = sign(1.,eigenvalue_x(2))
            
            mtx_x(1,1) = 0.5 * ( b + c )
            mtx_x(1,2) = 0.5 * ( c - b ) * sqrt(mesh%sqrtG(i,j,iPatch) * mesh%matrixIG(1,1,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ))
            mtx_x(2,1) = 0.5 * ( c - b ) / sqrt(mesh%sqrtG(i,j,iPatch) * mesh%matrixIG(1,1,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ))
            mtx_x(2,2) = mtx_x(1,1)
            ...
            dFlux = flux_x_ext_n(ip1,jc,iPatch) - flux_x_ext_p(ip1,jc,iPatch)
            dE    = E_ext_n     (ip1,jc,iPatch) - E_ext_p     (ip1,jc,iPatch)
            
            flux_x_ext(ip1,jc,iPatch) = 0.5 * ( flux_x_ext_p(ip1,jc,iPatch) + flux_x_ext_n(ip1,jc,iPatch) - mtx_x(1,1) * dFlux - mtx_x(1,2) * dE )
            E_ext     (ip1,jc,iPatch) = 0.5 * ( E_ext_p     (ip1,jc,iPatch) + E_ext_n     (ip1,jc,iPatch) - mtx_x(2,1) * dFlux - mtx_x(2,2) * dE )
        ...
        !$OMP PARALLEL DO PRIVATE(i,eigenvalue_y,b,c,mtx_y,jp1,jm1,ic,dE,dFlux)
        do j = jts-1, jte
          do i = its, ite
            eigenvalue_y(1) = stat%vc(i,j,iPatch) - sqrt( mesh%matrixIG(2,2,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ) )
            eigenvalue_y(2) = stat%vc(i,j,iPatch) + sqrt( mesh%matrixIG(2,2,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ) )
            
            b = sign(1.,eigenvalue_y(1))
            c = sign(1.,eigenvalue_y(2))
            
            mtx_y(1,1) = 0.5 * ( b + c )
            mtx_y(1,2) = 0.5 * ( c - b ) * sqrt(mesh%sqrtG(i,j,iPatch) * mesh%matrixIG(2,2,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ))
            mtx_y(2,1) = 0.5 * ( c - b ) / sqrt(mesh%sqrtG(i,j,iPatch) * mesh%matrixIG(2,2,i,j,iPatch) * ( stat%phi(i,j,iPatch) + mesh%phis(i,j,iPatch) ))
            mtx_y(2,2) = mtx_y(1,1)
            ...
            dFlux = flux_y_ext_n(ic,jp1,iPatch) - flux_y_ext_p(ic,jp1,iPatch)
            dE    = E_ext_n     (ic,jp1,iPatch) - E_ext_p     (ic,jp1,iPatch)
            
            flux_y_ext(ic,jp1,iPatch) = 0.5 * ( flux_y_ext_p(ic,jp1,iPatch) + flux_y_ext_n(ic,jp1,iPatch) - mtx_y(1,1) * dFlux - mtx_y(1,2) * dE )
            E_ext     (ic,jp1,iPatch) = 0.5 * ( E_ext_p     (ic,jp1,iPatch) + E_ext_n     (ic,jp1,iPatch) - mtx_y(2,1) * dFlux - mtx_y(2,2) * dE )
