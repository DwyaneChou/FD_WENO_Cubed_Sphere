MODULE spatial_operators_mod
  ...
  implicit none
    ...
    subroutine spatial_operator(stat,tend)
      ...
      real, dimension(ics:ice,jcs:jce,ifs:ife) ::  maxeigen_x
      real, dimension(ics:ice,jcs:jce,ifs:ife) ::  maxeigen_y
      ...
      do iPatch = ifs, ife
        ! Calculate eigenvalues on x direction
        !$OMP PARALLEL DO PRIVATE(i,eigenvalue_x)
        do j = jts, jte
          do i = its-1, ite+1
            eigenvalue_x(1,1) = 0.!stat%uc(i-1,j,iPatch) - sqrt( mesh%matrixIG(1,1,i-1,j,iPatch) * ( stat%phi(i-1,j,iPatch) + mesh%phis(i-1,j,iPatch) ) )
            eigenvalue_x(2,1) = 0.!stat%uc(i-1,j,iPatch) + sqrt( mesh%matrixIG(1,1,i-1,j,iPatch) * ( stat%phi(i-1,j,iPatch) + mesh%phis(i-1,j,iPatch) ) )
            ...
            eigenvalue_x(1,3) = 0.!stat%uc(i+1,j,iPatch) - sqrt( mesh%matrixIG(1,1,i+1,j,iPatch) * ( stat%phi(i+1,j,iPatch) + mesh%phis(i+1,j,iPatch) ) )
            eigenvalue_x(2,3) = 0.!stat%uc(i+1,j,iPatch) + sqrt( mesh%matrixIG(1,1,i+1,j,iPatch) * ( stat%phi(i+1,j,iPatch) + mesh%phis(i+1,j,iPatch) ) )
            
            maxeigen_x(i,j,iPatch) = maxval(abs(eigenvalue_x))
          enddo
        enddo
        !$OMP END PARALLEL DO
      enddo
      ...
      do iPatch = ifs, ife
        ! Calculate eigenvalues on y direction
        !$OMP PARALLEL DO PRIVATE(i,eigenvalue_y)
        do j = jts-1, jte+1
          do i = its, ite
            eigenvalue_y(1,1) = 0!stat%vc(i,j-1,iPatch) - sqrt( mesh%matrixIG(2,2,i,j-1,iPatch) * ( stat%phi(i,j-1,iPatch) + mesh%phis(i,j-1,iPatch) ) )
            eigenvalue_y(2,1) = 0!stat%vc(i,j-1,iPatch) + sqrt( mesh%matrixIG(2,2,i,j-1,iPatch) * ( stat%phi(i,j-1,iPatch) + mesh%phis(i,j-1,iPatch) ) )
            ...
            eigenvalue_y(1,3) = 0!stat%vc(i,j+1,iPatch) - sqrt( mesh%matrixIG(2,2,i,j+1,iPatch) * ( stat%phi(i,j+1,iPatch) + mesh%phis(i,j+1,iPatch) ) )
            eigenvalue_y(2,3) = 0!stat%vc(i,j+1,iPatch) + sqrt( mesh%matrixIG(2,2,i,j+1,iPatch) * ( stat%phi(i,j+1,iPatch) + mesh%phis(i,j+1,iPatch) ) )
            
            maxeigen_y(i,j,iPatch) = maxval(abs(eigenvalue_y))
          enddo
        enddo
        !$OMP END PARALLEL DO
      enddo
      
      call weno(flux_x_ext_p, flux_x + maxeigen_x * stat%phiG, 1, 1)
      call weno(flux_x_ext_n, flux_x - maxeigen_x * stat%phiG,-1, 1)
      call weno(E_ext_p     , E      + maxeigen_x * stat%u*20, 1, 1)
      call weno(E_ext_n     , E      - maxeigen_x * stat%u*20,-1, 1)
      call weno(v_ext_p     , stat%v                         , 1, 1)
      call weno(v_ext_n     , stat%v                         ,-1, 1)
      call weno(flux_y_ext_p, flux_y + maxeigen_y * stat%phiG, 1, 2)
      call weno(flux_y_ext_n, flux_y - maxeigen_y * stat%phiG,-1, 2)
      call weno(E_ext_p     , E      + maxeigen_y * stat%v*20, 1, 2)
      call weno(E_ext_n     , E      - maxeigen_y * stat%v*20,-1, 2)
      call weno(u_ext_p     , stat%u                         , 1, 2)
      call weno(u_ext_n     , stat%u                         ,-1, 2)
      
      do iPatch = ifs, ife
        !$OMP PARALLEL DO PRIVATE(i,ip1,jc)
        do j = jts, jte
          do i = its-1, ite
            ip1 = 2 * i + 1
            jc  = 2 * j
            
            flux_x_ext(ip1,jc,iPatch) = 0.5 * ( flux_x_ext_p(ip1,jc,iPatch) + flux_x_ext_n(ip1,jc,iPatch) )
            E_ext     (ip1,jc,iPatch) = 0.5 * ( E_ext_p     (ip1,jc,iPatch) + E_ext_n     (ip1,jc,iPatch) )
            ...
            !print*,flux_x_ext(ip1,jc,iPatch), flux_x(i,j,iPatch), maxeigen_x(i,j,iPatch) * stat%phiG(i,j,iPatch), flux_x_ext_p(ip1,jc,iPatch), flux_x_ext_n(ip1,jc,iPatch)
            !print*,E_ext(ip1,jc,iPatch), E(i,j,iPatch), maxeigen_x(i,j,iPatch) * stat%u(i,j,iPatch), E_ext_p(ip1,jc,iPatch), E_ext_n(ip1,jc,iPatch)
            !print*,v_ext(ip1,jc,iPatch),stat%uc(i,j,iPatch),v_ext_p     (ip1,jc,iPatch),v_ext_n     (ip1,jc,iPatch)
          enddo
        enddo
        !$OMP END PARALLEL DO
      enddo
      ...
      do iPatch = ifs, ife
        !$OMP PARALLEL DO PRIVATE(i,ic,jp1)
        do j = jts-1, jte
          do i = its, ite
            ic  = 2 * i
            jp1 = 2 * j + 1
            
            flux_y_ext(ic,jp1,iPatch) = 0.5 * ( flux_y_ext_p(ic,jp1,iPatch) + flux_y_ext_n(ic,jp1,iPatch) )
            E_ext     (ic,jp1,iPatch) = 0.5 * ( E_ext_p     (ic,jp1,iPatch) + E_ext_n     (ic,jp1,iPatch) )
