MODULE spatial_operators_mod
  ...
  implicit none
    ...
    subroutine spatial_operator(stat,tend)
      ...
      do iPatch = ifs, ife
        !$OMP PARALLEL DO PRIVATE(i,mtx_x,ip1,im1,jc)
        do j = jts, jte
          do i = its-1, ite
            ip1 = 2 * i + 1
            im1 = 2 * i - 1
            ...
            flux_x_ext(ip1,jc,iPatch) = 0.5 * ( flux_x_ext_p(ip1,jc,iPatch) + flux_x_ext_n(ip1,jc,iPatch) - sign(1.,stat%uc(i,j,iPatch)) * ( flux_x_ext_n(ip1,jc,iPatch) - flux_x_ext_p(ip1,jc,iPatch) ) )
            E_ext     (ip1,jc,iPatch) = 0.5 * ( E_ext_p     (ip1,jc,iPatch) + E_ext_n     (ip1,jc,iPatch) - sign(1.,stat%uc(i,j,iPatch)) * ( E_ext_n     (ip1,jc,iPatch) - E_ext_p     (ip1,jc,iPatch) ) )
        ...
        !$OMP PARALLEL DO PRIVATE(i,mtx_y,jp1,jm1,ic)
        do j = jts-1, jte
          do i = its, ite
            ic  = 2 * i
            jp1 = 2 * j + 1
            ...
            flux_y_ext(ic,jp1,iPatch) = 0.5 * ( flux_y_ext_p(ic,jp1,iPatch) + flux_y_ext_n(ic,jp1,iPatch) - sign(1.,stat%vc(i,j,iPatch)) * ( flux_y_ext_n(ic,jp1,iPatch) - flux_y_ext_p(ic,jp1,iPatch) ) )
            E_ext     (ic,jp1,iPatch) = 0.5 * ( E_ext_p     (ic,jp1,iPatch) + E_ext_n     (ic,jp1,iPatch) - sign(1.,stat%vc(i,j,iPatch)) * ( E_ext_n     (ic,jp1,iPatch) - E_ext_p     (ic,jp1,iPatch) ) )
